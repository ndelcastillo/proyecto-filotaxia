...
document.querySelectorAll(".gallery__item").forEach(item => {
    ...
    item.addEventListener("mouseenter", () => {
        // Texto
        gsap.to(original, { opacity: 0, duration: 0, ease: "power2.out" });
        gsap.to(alt, { opacity: 1, duration: 0, ease: "power2.out" });
    ...
    item.addEventListener("mouseleave", () => {
        gsap.to(original, { opacity: 1, duration: 0, ease: "power2.out" });
        gsap.to(alt, { opacity: 0, duration: 0, ease: "power2.out" });
